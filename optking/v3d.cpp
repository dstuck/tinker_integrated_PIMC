...
fprintf(outfile,"\n");
  ...
  // eBC
  if (! v3d_eAB(B, C, eBC) ) {
fprintf(outfile, "could not normalize eBC, \n B:");
for (int i=0; i<3; ++i) fprintf(outfile,"%15.10lf", B[i]);
fprintf(outfile,"\n C:");
for (int i=0; i<3; ++i) fprintf(outfile,"%15.10lf", C[i]);
fprintf(outfile,"\n eBC:");
for (int i=0; i<3; ++i) fprintf(outfile,"%15.10lf", eBC[i]);
fprintf(outfile,"\n");
    return false;
  }
