...
FRAG::FRAG(int natom_in, double *Z_in, double **geom_in) {
  ...
  globalIndex = init_int_array(natom);
   //cout << "DES: init frag" << endl;
...
void FRAG::set_default_masses(void) {
  int i;
  for (i=0; i<natom; ++i) {
   //cout << "DES in frag: Z = " << Z[i] << endl;
    mass[i] = Z_to_mass[(int) Z[i]];
   //cout << "DES: mass = " << mass[i] << endl;
  }
  return;
}
...
void FRAG::update_connectivity_by_distances(void) {
  ...
  for (i=0; i<natom; ++i) {
    for (j=0; j<i; ++j) {
      //Rij = v3d_dist(geom[globalIndex[i]], geom[globalIndex[j]]);
      Rij = v3d_dist(geom[i], geom[j]);
      //std::cout << "DES: Distance("<<i<<","<<j<<") = " << Rij << "\tvs.\t" <<  scale * (cov_radii[Zint[i]] + cov_radii[Zint[j]])/_bohr2angstroms << std::endl;
      if (Rij < scale * (cov_radii[Zint[i]] + cov_radii[Zint[j]])/_bohr2angstroms)
      //if (Rij < scale * (cov_radii[Zint[globalIndex[i]]] + cov_radii[Zint[globalIndex[j]]])/_bohr2angstroms)
...
//  }
  for (i=0; i<natom; ++i) {
    fprintf(fp,"\t %d :", globalIndex[i]+1);
      for (j=0; j<natom; ++j)
        if (connectivity[i][j]) fprintf(fp," %d", globalIndex[j]+1);
    fprintf(fp,"\n");
  }
  fprintf(fp,"\n");
  fflush(fp);
...
void FRAG::set_geom_array(double * geom_array_in) {
  ...
  for (i=0; i<natom; ++i)
    for (xyz=0; xyz<3; ++xyz)
      geom[i][xyz] = geom_array_in[3*globalIndex[i]+xyz];
}
...
double ** FRAG::inertia_tensor (GeomType in_geom) {
  ...
  center = com(in_geom);
   ...
   //cout << "DES: COM = " << center[0] << ","  << center[1] << ","  << center[2] << endl;

  int atom, xyz, xyz2;
  double tval;

//DES: This seems so very wrong
/*
  for (int atom=0; atom<g_natom(); ++atom) {
   //cout << "DES: mass in inertia = " << mass[atom] << endl;
    tval = 0.0;
    for (xyz=0; xyz<3; ++xyz) {
...
*/
 for (xyz=0; xyz<3; ++xyz) {
   for (xyz2=0; xyz2<3; ++xyz2) {
      ...
      for (int atom=0; atom<g_natom(); ++atom) {
        if (xyz == xyz2) {
           I[xyz][xyz2] += (in_geom[atom][xyz] - center[xyz]) * (in_geom[atom][xyz] - center[xyz]);
         }
        I[xyz][xyz2] -=   (in_geom[atom][xyz] - center[xyz]) * (in_geom[atom][xyz2] - center[xyz2]);
      }
    }
  }
...
double * FRAG::com(GeomType in_geom) {
  ...
  for (int i=0; i<g_natom(); ++i) {
    ...
    for (int xyz=0; xyz<3; ++xyz) {
      ...
      //cout << "DES: geom = " << in_geom[i][xyz] << endl;
    }
  }
   //cout << "DES: sum = " << sum << endl;
  for (int xyz=0; xyz<3; ++xyz) 
    center[xyz] /= sum;
...
int FRAG::principal_axes(GeomType in_geom, double **axes, double *evals) {
  ...
  for (int i=0; i<3; ++i) {
    if (fabs(I_evals[i]) > 1.0e-14) {
      cout << "DES: I_evals = " << I_evals[i] << endl;
      evals[cnt] = I_evals[i];
      axes[cnt][0] = I[i][0];
      axes[cnt][1] = I[i][1];
      axes[cnt][2] = I[i][2];
